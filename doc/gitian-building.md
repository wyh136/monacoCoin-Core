...
*Setup instructions for a Gitian build of monacoCoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the monacoCoin
...
- [Building monacoCoin Core](#building-monacoCoin-core)
...
Clone the git repositories for monacoCoin Core and Gitian.
...
in the monacoCoin Core repository under 'Fetch and build inputs' to install sources which require
...
To build monacoCoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the monacoCoin Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/monacoCoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/monacocoin-net/monacoCoin-Core
...
For example:
```bash
URL=https://github.com/crowning-/monacoCoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit monacoCoin=${COMMIT} --url monacoCoin=${URL} ../monacoCoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit monacoCoin=${COMMIT} --url monacoCoin=${URL} ../monacoCoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit monacoCoin=${COMMIT} --url monacoCoin=${URL} ../monacoCoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the monacoCoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../monacoCoin/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clonehttps://github.com/monacocoin-net/monacoCoin-Core.git

BTCPATH=/some/root/path/monacoCoin.git
SIGPATH=/some/root/path/monacoCoin-detached-sigs.git

./bin/gbuild --url monacoCoin=${BTCPATH},signature=${SIGPATH} ../monacoCoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/monacoCoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/monacoCoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/monacoCoin-osx-build.assert
```
...
[monacoCoin/gitian.sigs](https://github.com/monacocoin-net/monacoCoin-Core.sigs/) repository, or if that's not possible to create a pull
request.
