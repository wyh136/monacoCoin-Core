...
#if defined(NDEBUG)
# error "monacoCoin Core cannot be compiled without assertions."
#endif
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
   ...
   /* if (nPrevHeight <= 4500 && Params().NetworkIDString() == CBaseChainParams::MAIN) {
	...
	if(nPrevHeight <= 7200) {
		nSubsidyBase = 1000; // Gouvernance	MCC
	}else if((nPrevHeight > 12500) & (nPrevHeight <= 14000)){		
		nSubsidyBase = 1000; // Gouvernance	MCC
	}else {
		if((nPrevHeight > 7200) & (nPrevHeight <= 270000)) nSubsidyBase = 10; // 12 Months 262 800 blocks = 2.628 M		
		if((nPrevHeight > 270000) & (nPrevHeight <= 532800)) nSubsidyBase = 5; // 12 Months 262 800 blocks = 1.314 M
		if((nPrevHeight > 532800) & (nPrevHeight <= 1058400)) nSubsidyBase = 3; // 2 years 525 600 blocks = 1.576 M
		if((nPrevHeight > 1058400) & (nPrevHeight <= 1584000)) nSubsidyBase = 2; // 2 years 525 600 blocks = 1.576 M
		if(nPrevHeight > 1584000) nSubsidyBase = 1; // After	
	}
...
void ThreadScriptCheck() {
    RenameThread("monacoCoin-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // MCC : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(MCC): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(MCC): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // MCC : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(MCC): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(MCC): spork is off, skipping transaction locking checks\n");
    }
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /* 
        monacoCoin Related Inventory Messages
