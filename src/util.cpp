...
const char * const BITCOIN_CONF_FILENAME = "monacoCoin.conf";
const char * const BITCOIN_PID_FILENAME = "monacoCoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "monacoCoin" is a composite category enabling all monacoCoin-related debug output
            if(ptrCategory->count(string("monacoCoin"))) {
...
#else
    const char* pszModule = "monacoCoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\monacoCoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\monacoCoinCore
    // Mac: ~/Library/Application Support/monacoCoinCore
    // Unix: ~/.monacoCoinCore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "monacoCoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/monacoCoinCore";
#else
    // Unix
    return pathRet / ".monacoCoinCore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty monacoCoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override monacoCoin.conf
